...
for k in xrange(12):
    ...
    if k == 0:
        ...
        sum_tors_neut = neut_comp[k].groupby('time.dayofyear').sum('time').where(neut_comp[k].groupby('time.dayofyear').sum('time')/num_yrs_neut!='nan')
        sum_tors_nino = nino_comp[k].groupby('time.dayofyear').sum('time').where(nino_comp[k].groupby('time.dayofyear').sum('time')/num_yrs_nino!='nan')
        sum_tors_nina = nina_comp[k].groupby('time.dayofyear').sum('time').where(nina_comp[k].groupby('time.dayofyear').sum('time')/num_yrs_nina!='nan')
    
    elif k == 1:
        
        sum_tors_neut_2 = neut_comp[k].groupby('time.dayofyear').sum('time').where(neut_comp[k].groupby('time.dayofyear').sum('time')/num_yrs_neut!='nan')
        sum_tors_nino_2 = nino_comp[k].groupby('time.dayofyear').sum('time').where(nino_comp[k].groupby('time.dayofyear').sum('time')/num_yrs_nino!='nan')
        sum_tors_nina_2 = nina_comp[k].groupby('time.dayofyear').sum('time').where(nina_comp[k].groupby('time.dayofyear').sum('time')/num_yrs_nina!='nan')
...
gauss_peak_N = np.argmax(sliced_gaussN,axis=0)
gauss_peak_EN = np.argmax(sliced_gaussEN,axis=0)
gauss_peak_LN = np.argmax(sliced_gaussLN,axis=0)
...
maria_color = ({0.:'midnightblue',0.0832:'midnightblue',
                0.0833:'mediumblue',0.1665:'mediumblue',
                0.1666:'cornflowerblue',0.2499:'cornflowerblue',
                0.25:'lightsteelblue',0.3332:'lightsteelblue',
                0.3333:'darkgrey',0.4165:'darkgrey',
                0.4166:'darkgoldenrod',0.4999:'darkgoldenrod',
                0.5:'goldenrod',0.5832:'goldenrod',
                0.5833:'lightsalmon',0.6665:'lightsalmon',
                0.6666:'salmon',0.7499:'salmon',
                0.75:'red',0.8332:'red',
                0.8333:'darkred',0.9165:'darkred',
                0.9166:'k',1.0:'k'})
...
cs = ax1.pcolormesh(x1[:-1], y1[:-1], g_mask2, vmin=0, vmax=366, cmap=cmap)

m.drawcoastlines()
m.drawstates()
m.drawcountries()
m.drawparallels(np.arange(int(20),int(51),5),labels=[1,0,0,0], linewidth=0.01, fontsize=11)
#m.drawmeridians(np.arange(int(-120),int(-65),10),labels=[0,0,0,1], linewidth=0.01)
...
cs = ax2.pcolormesh(x1[:-1], y1[:-1], g_mask3, vmin=0, vmax=366, cmap=cmap)

m.drawcoastlines()
m.drawstates()
m.drawcountries()
#m.drawparallels(np.arange(int(20),int(51),5),labels=[1,0,0,0], linewidth=0.01)
#m.drawmeridians(np.arange(int(-120),int(-65),10),labels=[0,0,0,1], linewidth=0.01)
...
cs = ax3.pcolormesh(x1[:-1], y1[:-1], g_mask2, vmin=0, vmax=366, cmap=cmap)
...
cs3 = ax4.pcolormesh(x1[:-1], y1[:-1], g_mask2-g_mask3, vmin=-100, vmax=100, cmap=plt.cm.get_cmap('bwr'))
...
cs2 = ax5.pcolormesh(x1[:-1], y1[:-1], g_mask2, vmin=0, vmax=366, cmap=cmap)
...
cs = ax6.pcolormesh(x1[:-1], y1[:-1], g_mask2-g_mask3, vmin=-100, vmax=100, cmap=plt.cm.get_cmap('bwr'))
...
ticks_1 = [0,61,122,183,245,305]
tick_1 = ['Jan','Mar','May','Jul','Sep','Nov']
...
#cbar.ax.set_xticklabels(ticks_1)
cbar.ax.tick_params(labelsize=11)
cbar.set_label('Tornado Report Peak Fraction Difference', fontsize=12) 
...
for i, j in product(xrange(len(sliced_gauss[0,:,0])),xrange(len(sliced_gauss[0,0,:]))):

    print np.nonzero(sliced_gauss[:,i,j].max())

gauss_data = xr.Dataset({'gauss':(['dayofyear','y','x'],sliced_gauss)}, 
                         coords={'dayofyear':sum_tors.dayofyear.values}) 

gauss_data.gauss.max(dim='dayofyear').values

index_gauss = np.array([[gauss_data.gauss.dayofyear.values for i in xrange(len(gauss_data.gauss.max(dim='dayofyear').values[:,0]))] for j in xrange(len(gauss_data.gauss.max(dim='dayofyear').values[0,:]))])


sliced_gauss[sliced_gauss[:,i,j].max()]

X, Y = np.meshgrid(latlon.lons.values, latlon.lats.values)
cs = m.pcolormesh(X, Y, gauss_smooth, vmin=0, vmax=0.03, cmap=plt.cm.get_cmap('Reds'))
...
g_mask = np.ma.masked_array(gauss_smooth_LN, mask_help==0)

x1, y1 = latlon.lons.values, latlon.lats.values
#cs = plt.contourf(x1[:-1], y1[:-1], g_mask, vmin=0, vmax=366, cmap=cmap, origin = 'lower')
cs = plt.pcolormesh(x1[:-1], y1[:-1], g_mask, vmin=0, vmax=366, cmap=cmap)
...
cbar = fig.colorbar(cs, cax=cbar_ax, ticks=ticks_1, orientation='vertical')

normed = mpl.colors.Normalize(vmin=0, vmax=366)
...
cbar.ax.set_yticklabels(tick_1)
