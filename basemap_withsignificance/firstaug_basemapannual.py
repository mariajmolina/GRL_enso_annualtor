...
sliced_gaussN = Gauss_SmoothN[(len(sum_tors[:,0,0])+32):(len(sum_tors[:,0,0])+214),:,:]
sliced_gaussEN = Gauss_SmoothEN[(len(sum_tors[:,0,0])+32):(len(sum_tors[:,0,0])+214),:,:]
sliced_gaussLN = Gauss_SmoothLN[(len(sum_tors[:,0,0])+32):(len(sum_tors[:,0,0])+214),:,:]
...
maria_color = ({0.:'midnightblue',
                0.1:'midnightblue',
                0.2:'mediumblue',
                0.3625:'cornflowerblue',
                0.425:'lightsteelblue',
                0.4875:'darkgrey',
                0.55:'darkgoldenrod',
                0.6125:'goldenrod',
                0.65:'lightsalmon',
...
mask_help = sum_tors[32:214,:,:].sum(dim='dayofyear')
g_mask2 = np.ma.masked_array(g_mask, mask_help==0)

cs = ax1.pcolormesh(x1[:-1], y1[:-1], g_mask2, vmin=0, vmax=183, cmap=cmap)
...
cs = ax2.pcolormesh(x1[:-1], y1[:-1], g_mask3, vmin=0, vmax=183, cmap=cmap)
...
cs = ax3.pcolormesh(x1[:-1], y1[:-1], g_mask2, vmin=0, vmax=183, cmap=cmap)
...
cs2 = ax5.pcolormesh(x1[:-1], y1[:-1], g_mask2, vmin=0, vmax=183, cmap=cmap)
...
tick_1 = ['Feb','Mar','Apr','May','Jun','Jul']
...
plt.savefig('wut_32.png', bbox_inches='tight', dpi=200)
