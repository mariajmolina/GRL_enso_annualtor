...
for _ in xrange(10000):
    ...
    try: 
        ...
        try:
            ...
            neut_concat[_,:,:] = np.ndarray.argmax(neut_file[:364,:,:], axis=0)
            nina_concat[_,:,:] = np.ndarray.argmax(nina_file[:364,:,:], axis=0)
            nino_concat[_,:,:] = np.ndarray.argmax(nino_file[:364,:,:], axis=0)

        except ValueError:
            
            try:
                
                neut_concat[_,:,:] = np.ndarray.argmax(neut_file[:363,:,:], axis=0)
                nina_concat[_,:,:] = np.ndarray.argmax(nina_file[:363,:,:], axis=0)
                nino_concat[_,:,:] = np.ndarray.argmax(nino_file[:363,:,:], axis=0)

            except ValueError:

                neut_concat[_,:,:] = np.ndarray.argmax(neut_file[:362,:,:], axis=0)
                nina_concat[_,:,:] = np.ndarray.argmax(nina_file[:362,:,:], axis=0)
                nino_concat[_,:,:] = np.ndarray.argmax(nino_file[:362,:,:], axis=0)                
...
ninaneut_diff = nina_concat - neut_concat
ninoneut_diff = nino_concat - neut_concat


nina_diff_upper = np.nanpercentile(ninaneut_diff,97.5,axis=0)
nina_diff_lower = np.nanpercentile(ninaneut_diff,2.5,axis=0)
nino_diff_upper = np.nanpercentile(ninoneut_diff,97.5,axis=0)
nino_diff_lower = np.nanpercentile(ninoneut_diff,2.5,axis=0)



g_mask = np.ma.masked_invalid(nina_diff_upper)
mask_help = sum_group_neut.sum(dim='dayofyear')
g_mask1 = np.ma.masked_array(g_mask, mask_help==0)

g_mask = np.ma.masked_invalid(nina_diff_lower)
mask_help = sum_group_neut.sum(dim='dayofyear')
g_mask2 = np.ma.masked_array(g_mask, mask_help==0)

g_mask = np.ma.masked_invalid(nino_diff_upper)
mask_help = sum_group_neut.sum(dim='dayofyear')
g_mask3 = np.ma.masked_array(g_mask, mask_help==0)

g_mask = np.ma.masked_invalid(nino_diff_lower)
mask_help = sum_group_neut.sum(dim='dayofyear')
g_mask4 = np.ma.masked_array(g_mask, mask_help==0)
...
for i, j in product(xrange(len(g_nino[:,0])),xrange(len(g_nino[0,:]))):
    ...
    if np.isfinite(g_nino[i,j]):
        ...
        if g_nino[i,j] >= nino_diff_upper[i,j]:
            
            nino_sign[i,j] = 1
            
        if g_nino[i,j] <= nino_diff_lower[i,j]:

            nino_sign[i,j] = 1

    if np.isfinite(g_nina[i,j]):
        
        if g_nina[i,j] >= nina_diff_upper[i,j]:
            
            nina_sign[i,j] = 1
            
        if g_nina[i,j] <= nina_diff_lower[i,j]:
...
x, y = x1, y1
scat_lats = np.zeros(len(y1)-1)
scat_lons = np.zeros(len(x1)-1)
for a in xrange(len(scat_lats)):
    scat_lats[a] = np.nanmean([y1[a],y1[a+1]])
for b in xrange(len(scat_lons)):
    scat_lons[b] = np.nanmean([x1[b],x1[b+1]])
...
'''with open('neut_diffs_concata_1', 'rb') as f:
     neut_diff_concata_1 = pickle.load(f)

with open('neut_diffs_concato_1', 'rb') as f:
     neut_diff_concato_1 = pickle.load(f)

with open('nina_diffs_concat_a_1', 'rb') as f:
     nina_diff_concat_a_1 = pickle.load(f)

with open('nino_diffs_concatao_1', 'rb') as f:
     nino_diff_concatao_1 = pickle.load(f)
     
with open('neut_diffs_concata_2', 'rb') as f:
     neut_diff_concata_2 = pickle.load(f)

with open('neut_diffs_concato_2', 'rb') as f:
     neut_diff_concato_2 = pickle.load(f)

with open('nina_diffs_concat_a_2', 'rb') as f:
     nina_diff_concat_a_2 = pickle.load(f)

with open('nino_diffs_concatao_2', 'rb') as f:
     nino_diff_concatao_2 = pickle.load(f)

with open('neut_diffs_concata_3', 'rb') as f:
     neut_diff_concata_3 = pickle.load(f)

with open('neut_diffs_concato_3', 'rb') as f:
     neut_diff_concato_3 = pickle.load(f)

with open('nina_diffs_concat_a_3', 'rb') as f:
     nina_diff_concat_a_3 = pickle.load(f)

with open('nino_diffs_concatao_3', 'rb') as f:
     nino_diff_concatao_3 = pickle.load(f)

with open('neut_diffs_concata_4', 'rb') as f:
     neut_diff_concata_4 = pickle.load(f)

with open('neut_diffs_concato_4', 'rb') as f:
     neut_diff_concato_4 = pickle.load(f)

with open('nina_diffs_concat_a_4', 'rb') as f:
     nina_diff_concat_a_4 = pickle.load(f)

with open('nino_diffs_concatao_4', 'rb') as f:
     nino_diff_concatao_4 = pickle.load(f)

with open('neut_diffs_concata_5', 'rb') as f:
     neut_diff_concata_5 = pickle.load(f)

with open('neut_diffs_concato_5', 'rb') as f:
     neut_diff_concato_5 = pickle.load(f)

with open('nina_diffs_concat_a_5', 'rb') as f:
     nina_diff_concat_a_5 = pickle.load(f)

with open('nino_diffs_concatao_5', 'rb') as f:
     nino_diff_concatao_5 = pickle.load(f)

with open('neut_diffs_concata_6', 'rb') as f:
     neut_diff_concata_6 = pickle.load(f)

with open('neut_diffs_concato_6', 'rb') as f:
     neut_diff_concato_6 = pickle.load(f)

with open('nina_diffs_concat_a_6', 'rb') as f:
     nina_diff_concat_a_6 = pickle.load(f)

with open('nino_diffs_concatao_6', 'rb') as f:
     nino_diff_concatao_6 = pickle.load(f)

with open('neut_diffs_concata_7', 'rb') as f:
     neut_diff_concata_7 = pickle.load(f)

with open('neut_diffs_concato_7', 'rb') as f:
     neut_diff_concato_7 = pickle.load(f)

with open('nina_diffs_concat_a_7', 'rb') as f:
     nina_diff_concat_a_7 = pickle.load(f)

with open('nino_diffs_concatao_7', 'rb') as f:
     nino_diff_concatao_7 = pickle.load(f)

with open('neut_diffs_concata_8', 'rb') as f:
     neut_diff_concata_8 = pickle.load(f)

with open('neut_diffs_concato_8', 'rb') as f:
     neut_diff_concato_8 = pickle.load(f)

with open('nina_diffs_concat_a_8', 'rb') as f:
     nina_diff_concat_a_8 = pickle.load(f)

with open('nino_diffs_concatao_8', 'rb') as f:
     nino_diff_concatao_8 = pickle.load(f)

with open('neut_diffs_concata_9', 'rb') as f:
     neut_diff_concata_9 = pickle.load(f)

with open('neut_diffs_concato_9', 'rb') as f:
     neut_diff_concato_9 = pickle.load(f)

with open('nina_diffs_concat_a_9', 'rb') as f:
     nina_diff_concat_a_9 = pickle.load(f)

with open('nino_diffs_concatao_9', 'rb') as f:
     nino_diff_concatao_9 = pickle.load(f)

with open('neut_diffs_concata_10', 'rb') as f:
     neut_diff_concata_10 = pickle.load(f)

with open('neut_diffs_concato_10', 'rb') as f:
     neut_diff_concato_10 = pickle.load(f)

with open('nina_diffs_concat_a_10', 'rb') as f:
     nina_diff_concat_a_10 = pickle.load(f)

with open('nino_diffs_concatao_10', 'rb') as f:
     nino_diff_concatao_10 = pickle.load(f)

with open('neut_diffs_concata_11', 'rb') as f:
     neut_diff_concata_11 = pickle.load(f)

with open('neut_diffs_concato_11', 'rb') as f:
     neut_diff_concato_11 = pickle.load(f)

with open('nina_diffs_concat_a_11', 'rb') as f:
     nina_diff_concat_a_11 = pickle.load(f)

with open('nino_diffs_concatao_11', 'rb') as f:
     nino_diff_concatao_11 = pickle.load(f)

with open('neut_diffs_concata_12', 'rb') as f:
     neut_diff_concata_12 = pickle.load(f)

with open('neut_diffs_concato_12', 'rb') as f:
     neut_diff_concato_12 = pickle.load(f)

with open('nina_diffs_concat_a_12', 'rb') as f:
     nina_diff_concat_a_12 = pickle.load(f)

with open('nino_diffs_concatao_12', 'rb') as f:
     nino_diff_concatao_12 = pickle.load(f)

with open('neut_diffs_concata_13', 'rb') as f:
     neut_diff_concata_13 = pickle.load(f)

with open('neut_diffs_concato_13', 'rb') as f:
     neut_diff_concato_13 = pickle.load(f)

with open('nina_diffs_concat_a_13', 'rb') as f:
     nina_diff_concat_a_13 = pickle.load(f)

with open('nino_diffs_concatao_13', 'rb') as f:
     nino_diff_concatao_13 = pickle.load(f)

with open('neut_diffs_concata_14', 'rb') as f:
     neut_diff_concata_14 = pickle.load(f)

with open('neut_diffs_concato_14', 'rb') as f:
     neut_diff_concato_14 = pickle.load(f)

with open('nina_diffs_concat_a_14', 'rb') as f:
     nina_diff_concat_a_14 = pickle.load(f)

with open('nino_diffs_concatao_14', 'rb') as f:
     nino_diff_concatao_14 = pickle.load(f)

with open('neut_diffs_concata_15', 'rb') as f:
     neut_diff_concata_15 = pickle.load(f)

with open('neut_diffs_concato_15', 'rb') as f:
     neut_diff_concato_15 = pickle.load(f)

with open('nina_diffs_concat_a_15', 'rb') as f:
     nina_diff_concat_a_15 = pickle.load(f)

with open('nino_diffs_concatao_15', 'rb') as f:
     nino_diff_concatao_15 = pickle.load(f)

with open('neut_diffs_concata_16', 'rb') as f:
     neut_diff_concata_16 = pickle.load(f)

with open('neut_diffs_concato_16', 'rb') as f:
     neut_diff_concato_16 = pickle.load(f)

with open('nina_diffs_concat_a_16', 'rb') as f:
     nina_diff_concat_a_16 = pickle.load(f)

with open('nino_diffs_concatao_16', 'rb') as f:
     nino_diff_concatao_16 = pickle.load(f)

with open('neut_diffs_concata_17', 'rb') as f:
     neut_diff_concata_17 = pickle.load(f)

with open('neut_diffs_concato_17', 'rb') as f:
     neut_diff_concato_17 = pickle.load(f)

with open('nina_diffs_concat_a_17', 'rb') as f:
     nina_diff_concat_a_17 = pickle.load(f)

with open('nino_diffs_concatao_17', 'rb') as f:
     nino_diff_concatao_17 = pickle.load(f)

with open('neut_diffs_concata_18', 'rb') as f:
     neut_diff_concata_18 = pickle.load(f)

with open('neut_diffs_concato_18', 'rb') as f:
     neut_diff_concato_18 = pickle.load(f)

with open('nina_diffs_concat_a_18', 'rb') as f:
     nina_diff_concat_a_18 = pickle.load(f)

with open('nino_diffs_concatao_18', 'rb') as f:
     nino_diff_concatao_18 = pickle.load(f)

with open('neut_diffs_concata_19', 'rb') as f:
     neut_diff_concata_19 = pickle.load(f)

with open('neut_diffs_concato_19', 'rb') as f:
     neut_diff_concato_19 = pickle.load(f)

with open('nina_diffs_concat_a_19', 'rb') as f:
     nina_diff_concat_a_19 = pickle.load(f)

with open('nino_diffs_concatao_19', 'rb') as f:
     nino_diff_concatao_19 = pickle.load(f)

with open('neut_diffs_concata_20', 'rb') as f:
     neut_diff_concata_20 = pickle.load(f)

with open('neut_diffs_concato_20', 'rb') as f:
     neut_diff_concato_20 = pickle.load(f)

with open('nina_diffs_concat_a_20', 'rb') as f:
     nina_diff_concat_a_20 = pickle.load(f)

with open('nino_diffs_concatao_20', 'rb') as f:
