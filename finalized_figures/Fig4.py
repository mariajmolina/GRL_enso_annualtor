...
gauss_diff_ENs = gauss_smooth_EN - gauss_smooth_N
gauss_diff_LNs = gauss_smooth_LN - gauss_smooth_N
...
for i, j in product(xrange(len(g_nino[:,0])),xrange(len(g_nino[0,:]))):
    ...
    if np.isfinite(gauss_diff_ENs[i,j]):
        
        if gauss_diff_ENs[i,j] >= nino_diff_upper[i,j]:
            
            nino_sign[i,j] = 1
            
        if gauss_diff_ENs[i,j] <= nino_diff_lower[i,j]:

            nino_sign[i,j] = 1

    if np.isfinite(gauss_diff_LNs[i,j]):
        
        if gauss_diff_LNs[i,j] >= nina_diff_upper[i,j]:
            
            nina_sign[i,j] = 1
            
        if gauss_diff_LNs[i,j] <= nina_diff_lower[i,j]:
...
cs = ax1.pcolormesh(x3, y3, g_mask2, vmin=0, vmax=365, cmap=cmap)
...
cs = ax2.pcolormesh(x3, y3, g_mask3, vmin=0, vmax=365, cmap=cmap)
...
cs = ax3.pcolormesh(x3, y3, g_mask2, vmin=0, vmax=365, cmap=cmap)
...
cs2 = ax5.pcolormesh(x3, y3, g_mask2, vmin=0, vmax=365, cmap=cmap)
...
#plt.savefig('wut_4.pdf', bbox_inches='tight', dpi=200)

#plt.savefig('wut_4.png', bbox_inches='tight', dpi=200)
plt.savefig('wut_4.ps', bbox_inches='tight', dpi=200)
#plt.savefig('wut_4.pdf', bbox_inches='tight', dpi=200)
#plt.savefig('wut_4.eps', bbox_inches='tight', dpi=200)
