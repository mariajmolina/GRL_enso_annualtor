...
'''
Creation of maximum peak STP and/or maximum peak STP 3-hours figure.

Do not run full script at once. There are four shell sessions included below.

-First batch computes the STP / STP Hours data and gaussian smoothing for all years.

-Second batch parses the STP / STP Hours data into composite groups based on ONI.

-Third batch compiles the STP / STP Hours data into files with Julian day time coordinates.

-Fourth batch completes plotting of data. Notice the calls for
the significance files, which need to be created prior. Rewrite directory 
destinations to the appropriate new directories.

Choose whether you are analyzing STP or STP 3-Hours when loading 'data(s)' dataset.
...
###############################################################################
########################ALL YEARS MAXIMUM PEAK STP 3-Hours#####################
###############################################################################


## This is the script section for stp 3-hours data. Load the STP 3-Hours dataset for
##  all years previously created.
    

datas = xr.open_dataset('/storage/timme1mj/maria_pysplit/stphrs_ensoannualpaper', decode_cf=True)

num_yrs = len(np.unique(datas.coords['time'].dt.year))

sum_tors = datas.grid.resample(freq='D', dim='time', how='sum')

sum_tors = sum_tors.groupby('time.dayofyear').mean('time')
...
gauss_peak_AN_newversion = np.ndarray.argmax(sliced_gauss.values, axis=0)

with open('gauss_peak_AN_newversion', 'wb') as output:
    pickle.dump(gauss_peak_AN_newversion, output, pickle.HIGHEST_PROTOCOL)
...
#Choose STP or STP 3-Hours here.
    
data = xr.open_dataset('/storage/Reanalysis_Processed/stp_cin_1979_2016.nc', decode_cf=True)

data = xr.open_dataset('/storage/timme1mj/maria_pysplit/stphrs_ensoannualpaper', decode_cf=True)
...
#save files for next python session
...
#this was where I loaded stp 3-hours to do filtering... which the reasoning I am
#recalling doesn't make sense, would need to run this step-by-step to face issue
#again and recall.
...
#loading of precompiled stp files.
...
#significance before nan percentile
...
gauss_diff_EN = gauss_smooth_EN - gauss_smooth_N
gauss_diff_LN = gauss_smooth_LN - gauss_smooth_N
...
nino_sig = np.zeros(gauss_diff_EN.shape)
nina_sig = np.zeros(gauss_diff_EN.shape)

for i, j in product(xrange(len(gauss_diff_EN[:,0])),xrange(len(gauss_diff_EN[0,:]))):
        
    if np.isfinite(gauss_diff_EN[i,j]):
        
        if gauss_diff_EN[i,j] >= nino_upper[i,j]:
            
            nino_sig[i,j] = 1
            
        if gauss_diff_EN[i,j] <= nino_lower[i,j]:

            nino_sig[i,j] = 1
        
    if np.isfinite(gauss_diff_LN[i,j]):
        
        if gauss_diff_LN[i,j] >= nina_upper[i,j]:
            
            nina_sig[i,j] = 1
            
        if gauss_diff_LN[i,j] <= nina_lower[i,j]:
...
#loading of lat lon data that was precompiled.
...
new_lon = latlon.lons.values-0.16
new_lat = latlon.lats.values-0.16

latlon = latlon.assign(newlat=(['y','x'],new_lat))
latlon = latlon.assign(newlon=(['y','x'],new_lon))

x1, y1 = m(latlon.newlon.values, latlon.newlat.values)
...
#shapefile functions for filtering data outside of CONUS
...
#removal of data outside of CONUS
...
#this is the incorrect masking using stp 3-hours... needs to be vetted and thought out better.
...
#unknown reasoning for masking again... needs to be vetted.
...
ax4.scatter(x1, y1, g_mask_nino_mask, facecolors='k', edgecolors='k', linewidth=0.1, zorder=10)
...
ax6.scatter(x1, y1, g_mask_nina_mask, facecolors='k', edgecolors='k', linewidth=0.1, zorder=10)
...
cbar.set_label('Annual Peak Fraction of STP', fontsize=12) 
...
#plt.savefig('wut_3.ps', bbox_inches='tight', dpi=200)
#plt.savefig('wut_3.pdf', bbox_inches='tight', dpi=200)
plt.savefig('wut_3.eps', bbox_inches='tight', dpi=200)
